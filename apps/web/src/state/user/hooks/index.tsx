...
export function useGasPrice(chainIdOverride?: number): string {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.MONAD_TESTNET && chainId !== ChainId.MONAD_TESTNET,
    watch: true,
  })
...
export function toV2LiquidityToken([tokenA, tokenB]: [ERC20Token, ERC20Token]): ERC20Token {
  return new ERC20Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'Tektias LPs', 'Tektias-LP')
}
