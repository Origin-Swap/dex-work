...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowNativeBalance, onDismiss }) => {
  ...
  const bnbBalance = useBalance({ addressOrName: account, chainId: ChainId.MONAD_TESTNET })
  const nativeBalance = useBalance({ addressOrName: account, enabled: !isBSC })
  const native = useNativeCurrency()
  const wNativeToken = !isBSC ? WNATIVE[chainId] : null
  const wBNBToken = WNATIVE[ChainId.MONAD_TESTNET]
      ...
      <Box mb="24px">
        <Flex justifyContent="space-between" alignItems="center" mb="8px">
          <Flex bg={COLORS.BNB} borderRadius="16px" pl="4px" pr="8px" py="2px">
            <ChainLogo chainId={ChainId.MONAD_TESTNET} />
            <Text color="yellow" ml="4px">
              MONAD
            </Text>
          </Flex>
          <LinkExternal href={getBlockExploreLink(account, 'address', ChainId.MONAD_TESTNET)}>
            {getBlockExploreName(ChainId.MONAD_TESTNET)}
          </LinkExternal>
        </Flex>
        <Flex alignItems="center" justifyContent="space-between">
          <Text color="textSubtle">MONAD {t('Balance')}</Text>
        ...
        {wBNBBalance.gt(0) && (
          <Flex alignItems="center" justifyContent="space-between">
            <Text color="textSubtle">WMON {t('Balance')}</Text>
        ...
        <Flex alignItems="center" justifyContent="space-between">
          <Text color="textSubtle">{t('OFI Balance')}</Text>
