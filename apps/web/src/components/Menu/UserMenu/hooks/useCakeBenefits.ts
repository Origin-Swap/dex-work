...
const useCakeBenefits = () => {
  ...
  const currentBscBlock = useChainCurrentBlock(ChainId.MONAD_TESTNET)
  ...
  const { data, status } = useSWR(account && currentBscBlock && ['cakeBenefits', account], async () => {
    ...
    if (lockPosition === VaultPosition.Locked) {
      ...
      const poolAddresses = eligiblePools.map(({ contractAddress }) => getAddress(contractAddress, ChainId.MONAD_TESTNET))
      ...
      const [cakeVaultBalance, total] = await getScores(
        ...
        ChainId.MONAD_TESTNET.toString(),
        [account],
        currentBscBlock,
      )
      vCake = {
