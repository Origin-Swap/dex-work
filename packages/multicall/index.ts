...
export const multicallAddresses = {
  ...
  10143: '0x5654b448DeDaA4AAa85056f5Fef97c20def67C1D',
}
...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.MONAD_TESTNET) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.MONAD_TESTNET, options, provider: _provider }) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.MONAD_TESTNET, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
