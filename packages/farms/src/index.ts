...
const supportedChainId = [ChainId.MONAD_TESTNET]
...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  const fetchFarms = async (
    ...
    const masterChefAddress = isTestnet ? masterChefAddresses[ChainId.BSC] : masterChefAddresses[ChainId.MONAD_TESTNET]
  ...
  return {
    ...
    supportedChainId,
    isTestnet: (chainId: number) => ![ChainId.MONAD_TESTNET, ChainId.ETHEREUM].includes(chainId),
  }
}
